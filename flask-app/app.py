...
def survived_bar_chart(dataset, pass_class, cpalette=palette[1:3]):
    ...
    p = figure(tools=[hover_tool], height=400, title='Did/Did not Survive for Current Class')
...
def title_bar_chart(dataset, pass_class, cpalette=palette):
    ...
    p = figure(tools=[hover_tool], height=300, title='Titles for Current Class')
...
def age_hist_chart(dataset, pass_class, color=palette[1]):
    hist, edges = np.histogram(dataset[dataset['Pclass'] == int(pass_class)]['Age'].fillna(dataset['Age'].mean()), bins=25)
    ...
    p = figure(height=400, title='Age Histogram', tools=[hover_tool])
...
def redraw(p_class):
    ...
    title_chart = title_bar_chart(df, p_class)
    print("Chart 2 complete")
    age_chart = age_hist_chart(df, p_class)
...
@app.route('/')
def chart():
    survived_chart = survived_bar_chart(df, 1)
    title_chart = title_bar_chart(df, 1)
    age_chart = age_hist_chart(df, 1)
    ...
    script_hist_age, div_hist_age = components(age_chart)
    ...
    return render_template(
        ...
        div_hist_age_chart=div_hist_age,
        script_hist_age_chart=script_hist_age,
    )
